...
export const assignOffersToSegmentHandler = async (req: any) => {
  ...
  try {
    ...
    const segRes = await pool.query(
      `SELECT createdby, name FROM crm_segment WHERE id = $1 AND organization_id = $2`,
      [segmentId, orgId]
    );
    ...
    if (offerIds.length > 0) {
      const offerRes = await pool.query(
        `SELECT title FROM crm_offer WHERE id = ANY($1) AND organization_id = $2`,
        [offerIds, orgId]
      );
      if (offerRes.rows.length > 0) {
        offerTitles = offerRes.rows.map((row: any) => row.title).join(", ");
      }
    }
    ...
    try {
      ...
      if (offerIds.length > 0) {
        for (const offerId of offerIds) {
          await client.query(
            `INSERT INTO crm_segmentoffers (id, segment_id, offer_id, assigned_by_id, assigned_at, createdat, createdby, modifiedat, modifiedby)
             VALUES (gen_random_uuid(), $1, $2, $3, NOW(), NOW(), $3, NOW(), $3)`,
            [segmentId, offerId, userId]
          );
        }
      }
      ...
      await client.query(
        `INSERT INTO crm_segmentaudithistory (id, organization_id, segment_id, action_type, user_id, timestamp, details, createdat, createdby, modifiedat, modifiedby)
         VALUES (gen_random_uuid(), $1, $2, 'Assign Offers', $3, NOW(), $4, NOW(), $3, NOW(), $3)`,
        [orgId, segmentId, userId, `Assigned offers: [${offerTitles}] to Segment "${segment.name}".`]
      );
