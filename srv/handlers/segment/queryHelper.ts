...
export interface FilterRow {
  ...
  name?: string; // resolved filter type name
  operator: string;
  value: string;
  group_id?: string;
  logical_op?: string;
...
export const getFilterExpression = (name: string): string => {
  switch (name) {
    // Demographic
...
export const buildScopingSql = (user: any, params: any[]): string => {
...
export const buildFiltersQuery = (filters: FilterRow[], params: any[]): string => {
  if (!filters || filters.length === 0) return "1=1";

  // Group filters by group_id
  const groups: { [key: string]: FilterRow[] } = {};
  const ungrouped: FilterRow[] = [];

  for (const filter of filters) {
    if (filter.group_id) {
      if (!groups[filter.group_id]) groups[filter.group_id] = [];
  ...
  const compileFilterRow = (row: FilterRow): string => {
    const expr = getFilterExpression(row.name || "");
    if (!expr) return "1=1";
