...
export const deleteSegmentHandler = async (req: any) => {
  const { id } = req.data;
  ...
  try {
    ...
    if (userRole === "executive") {
      ...
      if (segment.createdby !== userId) {
        ...
        const execRes = await pool.query(`SELECT name FROM crm_user WHERE id = $1`, [segment.createdby]);
        ...
        await pool.query(
          `INSERT INTO crm_segmentaudithistory (id, organization_id, segment_id, action_type, user_id, timestamp, details, createdat, createdby, modifiedat, modifiedby)
           VALUES (gen_random_uuid(), $1, $2, 'Delete Override', $3, NOW(), $4, NOW(), $3, NOW(), $3)`,
          [orgId, id, userId, `Manager deleted Segment "${segment.name}" originally created by Executive "${execName}".`]
        );
      }
    }
    ...
    try {
      ...
      await client.query(
        `UPDATE crm_segmentaudithistory SET segment_id = NULL WHERE segment_id = $1`,
        [id]
      );
