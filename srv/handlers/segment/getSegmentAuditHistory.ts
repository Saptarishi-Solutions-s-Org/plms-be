...
export const getSegmentAuditHistoryHandler = async (req: any) => {
  ...
  try {
    ...
    const segRes = await pool.query(
      `SELECT createdby FROM crm_segment WHERE id = $1 AND organization_id = $2`,
      [segmentId, orgId]
    );
    ...
    const countRes = await pool.query(
      `SELECT COUNT(*) as total FROM crm_segmentaudithistory WHERE segment_id = $1 AND organization_id = $2`,
      [segmentId, orgId]
    );
    ...
    const logsRes = await pool.query(
      `SELECT ah.action_type, ah.timestamp, ah.details, u.name as username
