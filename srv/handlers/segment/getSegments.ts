...
export const getSegmentsHandler = async (req: any) => {
  ...
  try {
    ...
    if (is_active !== undefined && is_active !== null && String(is_active).trim() !== "") {
      const statuses = String(is_active).split(",").map((s: string) => s.trim().toLowerCase());
    ...
    for (const segment of listRes.rows) {
      ...
      const scopingSql = buildScopingSql(segmentOwnerUser, countParams);
      ...
      if (segment.type === "Static") {
        ...
        const conditionSql = buildFiltersQuery(resolvedFilters, countParams);
        ...
        `;
        const dynamicCountRes = await pool.query(dynamicCountQuery, countParams);
      ...
      const offerTitles = offersRes.rows.map((o: any) => o.title).join(", ");
