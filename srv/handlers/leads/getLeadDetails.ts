  ...
  try {
    ...
    const leadRes = await pool.query(
      `SELECT
       ...
       WHERE (l.id = $1 OR l.code = $2) AND l.organization_id = $3`,
    ...
    const segmentOffers: any[] = [];
    const segmentsRes = await pool.query(
      `SELECT id, type, name FROM crm_segment WHERE organization_id = $1 AND is_active = true`,
      [orgId]
    );
    
    const matchedSegmentIds: { id: string; name: string }[] = [];
    for (const seg of segmentsRes.rows) {
      if (seg.type === "Static") {
        const checkRes = await pool.query(
          `SELECT 1 FROM crm_segmentleads WHERE segment_id = $1 AND lead_id = $2`,
          [seg.id, leadId]
        );
    ...
    if (matchedSegmentIds.length > 0) {
      ...
      const segOffersRes = await pool.query(
        `SELECT 
           o.id        AS "offerId",
           o.title     AS "title",
           o.code      AS "code",
      ...
      const formatDateToYYYYMMDD = (val: any) => {
        if (!val) return null;
