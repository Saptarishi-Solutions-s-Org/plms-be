...
export const getReportManagerPerformanceHandler = async (req: any) => {
  try {
    ...
    if (search) {
      ...
      managerWhere.push(
        `(LOWER(manager.name) LIKE $${filterParams.length} OR LOWER(manager.email) LIKE $${filterParams.length} OR LOWER(manager.phone) LIKE $${filterParams.length})`,
      );
    }
    ...
    console.error(
      "Error fetching report manager performance:",
      error?.message ?? error,
    );
