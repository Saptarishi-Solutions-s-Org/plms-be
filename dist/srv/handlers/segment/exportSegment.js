...
exports.exportSegmentHandler = void 0;
...
const exportSegmentHandler = async (req) => {
    const { code } = req.data;
    ...
    try {
        ...
        const segRes = await db_1.pool.query(`SELECT id, name, type, createdby FROM crm_segment WHERE code = $1 AND organization_id = $2`, [code, orgId]);
        ...
        const creatorRes = await db_1.pool.query(`SELECT s.createdby, r.name as role_name
       FROM crm_segment s
       ...
       WHERE s.id = $1`, [segment.id]);
        ...
        const scopingSql = (0, queryHelper_1.buildScopingSql)(targetUser, params);
        ...
        if (segment.type === "Static") {
         ...
         WHERE sf.segment_id = $1`, [segment.id]);
            ...
            conditionSql = (0, queryHelper_1.buildFiltersQuery)(resolvedFilters, params);
        }
        const leadsQuery = `
      SELECT 
      ...
      ORDER BY l.name ASC
    `;
        const leadsRes = await db_1.pool.query(leadsQuery, params);
       ...
       VALUES (gen_random_uuid(), $1, $2, 'Export', $3, NOW(), $4, NOW(), $3, NOW(), $3)`, [orgId, segment.id, userId, `Exported ${leadsRes.rows.length} leads matching Segment "${segment.name}".`]);
        return { csvContent };
    }
    catch (err) {
...
exports.exportSegmentHandler = exportSegmentHandler;
