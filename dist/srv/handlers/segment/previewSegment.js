...
exports.previewSegmentHandler = void 0;
const db_1 = require("../../lib/db");
const queryHelper_1 = require("./queryHelper");
const previewSegmentHandler = async (req) => {
    ...
    try {
        ...
        if (segment_id) {
            const segRes = await db_1.pool.query(`SELECT s.createdby, r.name as role_name
         FROM crm_segment s
        ...
        if (type === "Static") {
            ...
            const filterTypeIds = filters.map((f) => f.filter_type_id).filter(Boolean);
            let resolvedFilters = [];
            if (filterTypeIds.length > 0) {
                const ftRes = await db_1.pool.query(`SELECT id, name FROM crm_segmentfiltertypes WHERE id = ANY($1)`, [filterTypeIds]);
...
exports.previewSegmentHandler = previewSegmentHandler;
