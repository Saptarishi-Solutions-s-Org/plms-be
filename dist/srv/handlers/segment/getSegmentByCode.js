...
exports.getSegmentByCodeHandler = void 0;
const db_1 = require("../../lib/db");
const getSegmentByCodeHandler = async (req) => {
    const { code } = req.data;
    ...
    try {
        ...
        const segRes = await db_1.pool.query(`SELECT id, code, name, description, type, is_active, color, notes, createdby 
       FROM crm_segment 
       WHERE code = $1 AND organization_id = $2`, [code, orgId]);
        ...
        if (userRole === "executive") {
            ...
            if (segment.createdby !== userId) {
                ...
                const creatorRes = await db_1.pool.query(`SELECT reporting_manager_id FROM crm_user WHERE id = $1`, [segment.createdby]);
        ...
        const filtersRes = await db_1.pool.query(`SELECT sf.id, sf.filter_type_id, sf.operator, sf.value, sf.group_id, sf.logical_op, ft.name as filter_type_name
       FROM crm_segmentfilters sf
       JOIN crm_segmentfiltertypes ft ON ft.id = sf.filter_type_id
       WHERE sf.segment_id = $1 
       ORDER BY sf.group_id ASC, sf.id ASC`, [segment.id]);
        const filters = filtersRes.rows.map((row) => ({
            id: row.id,
        ...
        const leadsRes = await db_1.pool.query(`SELECT lead_id FROM crm_segmentleads WHERE segment_id = $1`, [segment.id]);
        const static_lead_ids = leadsRes.rows.map((row) => row.lead_id);
        // 5. Fetch Assigned Offers
        const offersRes = await db_1.pool.query(`SELECT o.id, o.title, o.code 
       FROM crm_segmentoffers so
       JOIN crm_offer o ON o.id = so.offer_id
       WHERE so.segment_id = $1
       ORDER BY o.title ASC`, [segment.id]);
        const assigned_offers = offersRes.rows.map((row) => ({
            id: row.id,
            title: row.title,
            code: row.code
        }));
...
exports.getSegmentByCodeHandler = getSegmentByCodeHandler;
