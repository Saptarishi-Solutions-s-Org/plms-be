...
exports.exportOffersExecutiveHandler = void 0;
const db_1 = require("../../lib/db");
const exportOffersExecutiveHandler = async (req) => {
    try {
        const orgId = req.user?.orgId;
        const executiveId = req.user?.id;
        if (!orgId || !executiveId)
            return req.error(401, "Unauthorized");
        const res = await db_1.pool.query(`SELECT DISTINCT
         o.code                    AS "offerCode",
         o.title                   AS "title",
       ...
       ORDER BY o.createdat DESC`, [executiveId, orgId]);
        return res.rows;
    }
    catch (error) {
...
exports.exportOffersExecutiveHandler = exportOffersExecutiveHandler;
