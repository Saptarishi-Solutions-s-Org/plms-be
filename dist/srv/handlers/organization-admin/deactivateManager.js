...
exports.deactivateManagerHandler = void 0;
const db_1 = require("../../lib/db");
const socket_1 = require("../../realtime/socket");
const events_1 = require("../../realtime/events");
const deactivateManagerHandler = async (req) => {
    ...
    try {
        ...
        const managerRes = await client.query(`SELECT u.id, u.name, r.role_id
       FROM crm_user u
       ...
       AND LOWER(rl.name) = 'manager'`, [managerId, orgId]);
        ...
        const targetManagerRes = await client.query(`SELECT u.id, u.name
       FROM crm_user u
       JOIN crm_organizationroles r ON u.role_id = r.id
       JOIN crm_roles rl ON rl.id = r.role_id
       WHERE u.id = $1
       ...
       AND u.is_active = true`, [targetManagerId, orgId]);
        ...
        const executivesRes = await client.query(`SELECT u.id FROM crm_user u
       JOIN crm_organizationroles r ON u.role_id = r.id
       JOIN crm_roles rl ON rl.id = r.role_id
       WHERE u.reporting_manager_id = $1
       AND u.organization_id = $2
       AND LOWER(rl.name) = 'executive'`, [managerId, orgId]);
...
exports.deactivateManagerHandler = deactivateManagerHandler;
